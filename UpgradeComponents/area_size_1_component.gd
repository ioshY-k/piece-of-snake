...
func _ready() -> void:
	...
	tween.tween_property(level.current_map, "position", level.map_second_pos_and_scale[level.current_map_index][0], 0.8)
	tween.tween_property(level.current_map, "scale", level.map_second_pos_and_scale[level.current_map_index][1], 0.8)
