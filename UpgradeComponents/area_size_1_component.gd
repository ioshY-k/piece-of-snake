...
func _ready() -> void:
	...
	level.current_map.inbounds_grid_size = map_data_size1[level.current_map_index][2]
	...
	tween.tween_property(level.current_map, "position", map_data_size1[level.current_map_index][0], 0.8)
	tween.tween_property(level.current_map, "scale", map_data_size1[level.current_map_index][1], 0.8)
