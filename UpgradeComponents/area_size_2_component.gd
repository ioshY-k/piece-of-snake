...
func _ready() -> void:
	...
	level.current_map.inbounds_grid_size = MapData.get_map_data2(map_id)[2]
	...
	tween.tween_property(level.current_map, "position", MapData.get_map_data2(map_id)[0], 0.8)
	tween.tween_property(level.current_map, "scale", MapData.get_map_data2(map_id)[1], 0.8)
